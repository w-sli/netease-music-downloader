...
from core import UserError, atomic_json, read_json, safe_name
...
class DownloadManager:
    ...
    def enqueue(self, playlist, songs):
        with self.condition:
            ...
            for song in songs:
                ...
                key = str(folder / f"{song['id']}-{settings['quality']}")
                ...
                previous = next((j for j in reversed(self.jobs) if j.get("target_key") == key and (
                    j["status"] in ACTIVE or j["status"] == "queued"
                    or (j["status"] in {"completed", "skipped"} and Path(j.get("path") or "/nonexistent").is_file()))), None)
                if previous:
                    existing += 1
                    continue
    ...
    def _name_taken_by_another_song(self, job, target, folder):
        ...
        for other in self.jobs:
            if other["song_id"] == job["song_id"] or other["status"] not in CLAIMED:
                continue
            if other.get("path") == str(target):
                return True
            peer = other.get("song")
            if peer and Path(other.get("target_key") or "/").parent == folder:
    ...
    def _worker(self):
        while True:
            with self.condition:
                self.condition.wait_for(lambda: self.closed or (not self.paused and len(self.active) < self.store.settings["workers"] and any(j["status"] == "queued" for j in self.jobs)))
                if self.closed:
                    return
                ...
                self._save()
            try:
                for attempt in range(job["settings"]["retries"] + 1):
                    try:
                        self._check(event)
                        ...
                        if attempt < job["settings"]["retries"]:
    ...
    def _download(self, job, event):
        ...
        if target.exists():
            ...
            known = any(j.get("path") == str(target) and j["song_id"] == job["song_id"]
        ...
        warnings = []
        try:
            self._transfer(resource, temporary, job, event)
            self._check(event)
            ...
            if settings["cover"] and song.get("cover"):
                try:
                    with requests.get(song["cover"], timeout=(5, 12), stream=True) as r:
                        r.raise_for_status()
                        chunks = bytearray()
            ...
            # Hard linking refuses to overwrite files created after the existence check.
            os.link(temporary, target)
            temporary.unlink()
            if lyric_temp.exists():
                try:
                    os.link(lyric_temp, target.with_suffix(".lrc"))
    ...
    def _transfer(self, resource, target, job, event):
        digest = hashlib.md5()
        # Each worker owns its connection; cookies never go to the audio CDN.
        with requests.get(resource["url"], stream=True, timeout=(10, 15),
